...
singupForm.addEventListener('submit', (e) => {
    ...
    if(passkey == 'accelerando'){
        ...
        db.ref(`users/6Tvtz5BN65hHTd8kST4sFwVPGkm1/content`).push({
            ...
            db.ref(`users/${uid}`).set({info: {email: email}}).then(() => {
...
preFrontEditForm.addEventListener('submit', (e) => {
    e.preventDefault();
    db.ref(`users/Xw2NIcmtLOMvR8HgLzMUBYDBiYf2/content/${preFrontEditForm['group1'].value}`).once('value',(snap) => {
...
editfrontForm.addEventListener('submit', (e) => {
    ...
    db.ref(`users/Xw2NIcmtLOMvR8HgLzMUBYDBiYf2/content/${sec}`).set(editor.getContents()).then(() => {
...
createStudentForm.addEventListener('submit', (e) => {
    ...
    db.ref('users/' + userId + '/students').push({
        firstName: fName,
        lastName: lName,
        birthDate : bdate
      }).then(() => {
        ...
        db.ref(`users/6Tvtz5BN65hHTd8kST4sFwVPGkm1/content`).orderByChild('uid').once('value', snap => {
            snap.forEach((el) => {
                if(el.val().uid == userId){
                    ...
                    db.ref(`users/${userId}/students`).once('value', (snap2) => {
                        stu = [];
                        ...
                        stu.forEach(el2 => {
                            if(el2.firstName == fName){
                                db.ref(`users/6Tvtz5BN65hHTd8kST4sFwVPGkm1/content/${userPath}/students`).push(el2);
                            }
