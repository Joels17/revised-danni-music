				...
				if (el.val().uid == uid) {
					...
					for (var key in stus) {
						db.ref(
							...
							.orderByChild('uid')
							.once('value', (snap2) => {
								li = `<li><a href="student.html?studentId=${
									snap2.val().studentId
							...
							.then(() => {
								...
								if (
									html ==
									`<p id="logInfo">Click on a student to view their notes</p>`
								) {
					...
					if (
						html == `<p id="logInfo">Click on a student to view their notes</p>`
					) {
	...
	db.ref('users/6Tvtz5BN65hHTd8kST4sFwVPGkm1/content')
		...
		.once('value', (snap) => {
			...
			snap.forEach((element) => {
				...
				const li = `<li><a href="student.html?userId=${element.val().uid}">${
					element.val().email
				}</a></li>`;
